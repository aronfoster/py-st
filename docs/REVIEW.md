...
- Morning checks in a fresh Python 3.12 environment installed from `.[dev]`:
  **487 passed, 1 skipped**, Black/Ruff/mypy clean. Formatter versions now match
...
- Remote procurement is single-good/single-load. No arbitrary multi-load route
  optimizer, automatic ship purchasing, cross-system controller or fleet-wide
  concurrent execution is claimed.
