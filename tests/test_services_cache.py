...
from py_st import cache, services
...
def test_get_agent_info_cache_hit(
    mock_load_cache, mock_save_cache, mock_client_class
):
    ...
    mock_client_class.return_value.agent.get_agent.side_effect = (
        RuntimeError("API should not be called on cache hit")
    )
...
def test_get_agent_info_cache_miss_stale(
    ...
    assert time_diff < timedelta(
        seconds=5
    ), "Saved timestamp should be recent"
